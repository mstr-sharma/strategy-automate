...
SD_TECH_EXAMPLE_* tables in Synthetic Data / <snowflake-schema>.

Steps (all timed):
  1. Login (+ identityToken)
  2. Discover 4 tables matching SD_TECH_EXAMPLE in <snowflake-schema>
...
def main():
    ...
    assert len(tables) == 4, f"Expected 4 SD_TECH_EXAMPLE tables, got {len(tables)}"
    ...
    t = time.monotonic()
    # Find <sf-assignee> via metadata search — the user appears as the 'owner' of objects
    assignee_id = None
    r = m.get("/api/searches/results?name=assignee&getAncestors=false&limit=50")
    if r.ok:
        for it in r.json().get("result", []):
            o = it.get("owner") or {}
            if "assignee" in (o.get("name","").lower()) and ("<assignee-last>" in o.get("name","").lower()):
                assignee_id = o.get("id")
                print(f"[acl] found {o.get('name')} -> {assignee_id}", file=sys.stderr)
                break
    assignee_denied = False
    if not assignee_id:
        print("[acl] WARN <demo-user> not found; skipping deny", file=sys.stderr)
    else:
        # Fetch existing ACL, prepend a DENY entry for <sf-assignee>, PUT back.
        r = m.get(f"/api/objects/{derived_id}?type=4")
        r.raise_for_status()
        full = r.json()
        deny_mask = 1 | 64 | 128 | 512     # read+browse+execute+use
        deny_entry = {
            "trusteeId": assignee_id,
            "trusteeName": "<sf-assignee>, <sf-assignee>",
    ...
    print(json.dumps({
        ...
        "assignee_denied": assignee_denied,
        "published": published,
