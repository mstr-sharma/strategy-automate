...
repo at the current working directory. Your entry point is AGENTS.md. Memory lives
in memory/MEMORY.md (each line is a one-liner pointing to a typed memory file).
Skills live in skill/SKILL.md (build-mosaic-model), strategy-automation/SKILL.md
(NLQ router), strategy-validation/SKILL.md (data validation). Helper CLI scripts
are in skill/scripts/ and run with python3. Credentials come from env vars
(MSTR_BASE, MSTR_USER, MSTR_PASSWORD, MSTR_PROJECT_ID, MSTR_DEST_FOLDER_ID).
...
- **Context window**: load `memory/MEMORY.md` + the 2–3 memory files relevant to the task. Don't attempt to load all 60+ files unless the context budget exceeds 128k tokens.
