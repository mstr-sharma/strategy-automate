...
- **Tool use**: any Ollama model with function-calling (`llama3.2:3b-instruct`, `llama3.3:70b`, `qwen2.5-coder:32b`, `devstral:24b`, `hermes3`, etc.) can call `run_shell(cmd)` to invoke `python3 skills/build-mosaic-model/scripts/build_mosaic.py ...`. Models without function-calling can still output shell commands and let the harness execute them.
