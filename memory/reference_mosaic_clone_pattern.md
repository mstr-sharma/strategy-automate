...
- `8004ccfc` — duplicate model name in folder. Delete the old one via `DELETE /api/objects/{oldMID}?type=3` (the `/api/model/dataModels/{id}` DELETE path does NOT work on <tenant>.strategy.com — uses the generic objects endpoint).
...
Per `reference_mosaic_publish_path.md`: `POST /api/cubes/{MID}?cubeAction=publish` is the reliable trigger on <tenant>.strategy.com (matches the UI). Poll via the Modeling-native 3-step flow if you need per-table confirmation.
