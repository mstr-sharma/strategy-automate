...
description: Verified endpoint paths and payload shapes for the <tenant>.strategy.com MicroStrategy REST API; covers auth, datasources, catalog, data models, changesets, metrics, filters, transformations, security, translations, VLDB.
type: reference
originSessionId: cef55f31-c57d-4220-b4dc-eddfff684771
---
All paths prefixed with `{BASE} = https://<tenant>.strategy.com/MicroStrategyLibrary`. Unless otherwise noted, send `X-MSTR-AuthToken`, `X-MSTR-ProjectID`, and (for writes) `X-MSTR-IdentityToken` + `X-MSTR-MS-Changeset`.
...
- `api-docs/swagger-config` 404s on <tenant>.strategy.com; use `openapi-summary` in the helper.
...
- `POST /api/cubes/{id}` — verified <tenant>.strategy.com publish path for in-memory Mosaic models.
...
- Global `POST /api/objects/{objId}/acl` is not accepted on <tenant>.strategy.com for the tested object-security use case; use the data-model ACL endpoint when object belongs to a Mosaic model.
