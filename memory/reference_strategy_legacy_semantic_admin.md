...
Second verified chain same day (Year→Quarter→Month on the operator's Workstation-made prompted filter) added three lessons: (1) **a filter whose prompt is EMBEDDED (`isEmbedded: true`) cascades identically to a standalone-prompt filter** when referenced from `question.filter`; (2) **attribute resolution trap** — `/api/searches/results?type=12` returns managed/template attributes (empty ancestors) under common names like "Quarter"/"Month"; prompt creates accept them silently and only report create catches it (`8004da0c`) — resolve schema attributes from `/api/model/systemHierarchy` or an existing report's `dataTemplate.units`; (3) **`PUT /api/model/prompts/{id}` is full-replace and works for retargeting `question.attribute`** (echo information/title/instruction/question/restriction) — the prompt's objectId survives, so wrapping filters need no change. Also: prompts created by Workstation's filter editor default to `required: false` — check the runtime `required` flag when a level must not be skippable.
