...
- On <tenant>.strategy.com, if token-based compound metrics fail on commit, use the known fallback: a fact metric with an inline column formula using `character` operator tokens (`TOTAL_COST`, `/`, `QUANTITY_ORDERED`) and an aggregate function such as `avg`.
