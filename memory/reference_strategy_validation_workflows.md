...
- kept requested artifacts: `Books_secFilter-codex` (`6F6705C3935C409FB3BFB8BCFDDC177C`) assigned to `<operator-login>_duplicate-codex` (`6C5374CACE46881669DD40819EAF4B99`)
...
Core steps:
- Resolve project, source user `<operator>`, target username `<operator-login>_duplicate-codex`.
- Resolve `Category` attribute and `Books` element.
- Create or reuse `Books_secFilter-codex` via `/api/model/securityFilters`.
- Duplicate user with `POST /api/users?sourceUserId=<operatorId>` if `<operator-login>_duplicate-codex` does not already exist.
...
Core steps:
- Distribution: `GET /api/subscriptions`, `GET /api/schedules`, list addresses/recipients visible to `<operator>`.
