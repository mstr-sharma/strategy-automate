...
- [Relationship PUT is destructive](feedback_mosaic_relationship_put_wipes.md) — `PUT /attributes/{id}/relationships` replaces the full set in BOTH directions; use `put_relationships_merged()` (default in `wire-relationships`) or `--replace` to opt into the wipe.
