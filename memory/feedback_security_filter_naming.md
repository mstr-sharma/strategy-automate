...
description: Every Mosaic / classic security filter created by automation must have a name that describes WHAT the filter restricts. Generic names like "SF <sf-assignee>" or "Row-level filter" are not acceptable. Use the qualification itself as the name: "{Attribute} {operator} {value}" (e.g. "Tenant = Acme Compute", "Region IN (EMEA, APAC)", "Category_ID <= 5").
type: feedback
---
...
**Why:** SFs are reused across users, groups, and audits. A name keyed to "who" (e.g., "SF <sf-assignee> access") goes stale the moment the membership changes and hides the security rule from reviewers. Names keyed to "what" ("Tenant = Acme Compute") remain accurate for the life of the qualification.
...
- Prefer the human-readable display value, not the element ID (e.g., "Tenant = Acme Compute", not "Tenant.TENANT_ID = hAcme Compute"). The element-ID / form binding is an implementation detail of the REST payload, not a consumer-facing name.
