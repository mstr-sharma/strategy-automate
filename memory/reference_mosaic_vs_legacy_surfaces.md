...
The helper's current `/api/cubes/{id}` fallback is actually correct for Mosaic on <tenant>.strategy.com — the UI uses the same path. What's missing is a **post-202 confirmation step**. Required fix:
...
Publishing ANY Mosaic model in-memory on the Studio tenant — including a pre-existing, unrelated 6-table model (`<unrelated-test-model>`) — returned the same QueryEngineServer stall:
