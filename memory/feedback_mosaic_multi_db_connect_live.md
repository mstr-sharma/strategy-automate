...
- In_memory publish is currently failing tenant-wide on <tenant>.strategy.com with `iServerCode -2147212544` (parallel-mode stall; see `reference_mosaic_vs_legacy_surfaces.md` + `feedback_studio_tenant_publish_stall_2026_04_23.md`). Plan for a validation fallback that does not depend on the published Mosaic model.
