...
**How to apply:** Collapse the post-build steps (relationships via PUT, publish, SF create + commit, SF member assignment, <sf-assignee>-style user lookups, validation queries) into one Python script that opens one `requests.Session()` at the top and reuses it for everything. Pattern:
...
- Admin `kill-sessions` via `/api/monitors/...` is privileged and not available in <operator-login>-style operator accounts on studio. Don't try it blindly.
