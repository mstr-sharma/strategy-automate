...
- The pipeline JSON shape that works (see `_make_pipeline_table_body()`, used by `cmd_build`, in `skill/scripts/build_mosaic.py`) — generate **fresh UUIDs for every inner id** on each build:
  ```json
  {
    "id": "<uuid>",
    "rootTable": {
