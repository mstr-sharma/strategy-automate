...
1. **Scan for concrete identifiers.** Grep the draft for:
   - **Tenant / environment**: hostnames (`<tenant>.customer.cloud.microstrategy.com`, `<tenant>.strategy.com`, `env-*`, `tutorial.*`), project names (`Shared Studio`, `MicroStrategy Tutorial`, `Shared Reports/…`).
   - **Warehouse**: DB instance names (`Neon …`, `<snowflake-db> …`, `Snowflake Prod`), schema names (`<snowflake-db>`, `public`, `SALES`), table names (`USAGE_HOURLY`, `TENANTS`, `incidents`).
