...
1. **Scan for concrete identifiers.** Grep the draft for: tenant hostnames (`<tenant>.customer.cloud.microstrategy.com`, `<tenant>.strategy.com`, `env-*`, `tutorial.*`), project names (`Shared Studio`, `MicroStrategy Tutorial`, `Shared Reports/…`), DB instance names (`Neon …`, `<snowflake-db> …`, `Snowflake Prod`), schema names (`<snowflake-db>`, `public`, `SALES`), table names (`USAGE_HOURLY`, `TENANTS`, `incidents`), user identifiers (`<operator-user>`, real full names, email addresses, user IDs), model / report / folder / schedule / address IDs (32-hex strings), domain-specific entity names (`<tenant-a>`, `Lumina`, `customer_id`).
