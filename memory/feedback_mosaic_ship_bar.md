...
- **Post-build form-category PATCH is fragile on {MSTR_BASE host}.** `PATCH /api/model/dataModels/{mid}/attributes/{aid}` with `{forms:[...]}` returns `8004cc63 "Attribute does not contain a form with the ID"` even when GET returns that exact ID. Fix form names at **CREATE time**, not via post-hoc PATCH.
  - **Root cause found 2026-08-19 (DevOpsCo form-removal):** the 404/8004cc63 fires when something else on the attribute still REFERENCES a form id absent from the PATCHed `forms` array — `displays.reportDisplays/browseDisplays` (and possibly `sorts`/`keyForm`). Removing a form works when the PATCH body carries `forms` AND the corrected `displays` **in one atomic call**; PATCHing forms first and displays second fails validation on the first call. Appending a form never trips this.
