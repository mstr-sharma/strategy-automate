...
- **`dataServeMode` distribution:** 119 `in_memory`, 22 `connect_live`, 15 blank (legacy Hyper/MTDI datasets: Finanzas, <telematics-customer> Hyper, Customer Hyper, Agents Hyper, <retailer> Sample Retail Data, <hpc-vendor> Tenant/Cluster Hyper Dataset, etc.).
...
- **External data models:** 12 models reference others (<card-issuer> Comprehensive Analysis Suite references 5+ models; Unified Supplier Model, <industrial-customer> Integrated Asset Health, Cardmember Rewards Model, etc.).
