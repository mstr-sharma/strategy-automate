...
`GET /api/users?limit=N` returns `{}` empty even for admin. To find a user by name, search for any object they own via `/api/searches/results?name=<firstName>&limit=50` and read the `result[].owner.{id,name}` fields. Verified: `<sf-assignee>, <sf-assignee>` has user id `DBE00854E14B8D8919D3FBADCA61894B`.
...
- Keep the older failures below as evidence of endpoints that do not work on <tenant>.strategy.com for this use case.
