...
- Skills live as `SKILL.md` files with YAML frontmatter. Codex does not implement Anthropic's skills protocol natively; treat each `SKILL.md` as a long-form instruction file loaded on demand when the task matches its `description` field. The authoritative skill list and precedence chain are in `AGENTS.md` (cold-start routing) — do not rely on any list here.
