-- =====================================================================================
-- Post-load consistency suite for the DAI extension tables (run in Neon PG, psql).
-- Violation checks return 0 rows on a correct load; profile checks are for eyeballing.
-- =====================================================================================

-- 0. Row inventory
SELECT 'DAI_TEAM_APP_MONTHLY_COSTS' AS t, count(*) AS rows_,
       count(DISTINCT "TEAM_ID") AS teams, count(DISTINCT "APP_ID") AS apps
FROM "DAI_TEAM_APP_MONTHLY_COSTS"
UNION ALL
SELECT 'DAI_CUSTOMER_IMPACT', count(*), NULL, count(DISTINCT "APP_ID")
FROM "DAI_CUSTOMER_IMPACT";

-- 1. First-of-month convention (0 rows expected)
SELECT count(*) AS not_first_of_month
FROM "DAI_TEAM_APP_MONTHLY_COSTS" WHERE extract(day FROM "COST_MONTH") <> 1;
