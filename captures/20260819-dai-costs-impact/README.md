# DAI finance & customer-impact extension — 2026-08-19

Extends the Studio Mosaic model **"Deployment and Incident Analytics"**
(`CD3ACE32C83E48D5B28D6D790111C7B2`, Shared Studio > My Reports > DAI,
`dataServeMode: in_memory`) with two Neon PostgreSQL tables. Same pattern as
the Kobo extension (`captures/20260819-kobo-subscriptions-promos/`).
...
| --- | --- | --- |
| `DAI_TEAM_APP_MONTHLY_COSTS` | team × application × month, activity-based (a row per month the team deployed that app; `COST_MONTH` = first of month) | **Cloud Infrastructure Cost** = SUM(CLOUD_COST), **Engineering Hours Logged** = SUM(ENGINEERING_HOURS) |
| `DAI_CUSTOMER_IMPACT` | customer-impacting incident (≤1 per INCIDENTS row, `INCIDENT_ID` unique) | **Customers Affected** = SUM(CUSTOMERS_AFFECTED) |
...
fan-trap risk as the Kobo date split — unify when doing the best-practice pass).
