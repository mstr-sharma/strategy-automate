-- =====================================================================================
-- DAI software-delivery performance -- finance & customer extension tables (PostgreSQL / Neon)
--
...
-- TEAMS / APPLICATIONS in Snowflake MCI_DEMO, with 4 metrics: Lead Time Hours,
...
--   DAI_TEAM_APP_MONTHLY_COSTS  (team x application x month, activity-based:
--                                a row exists for each month the team deployed
--                                that app; ties spend to real delivery activity)
--   DAI_CUSTOMER_IMPACT         (one row per customer-impacting incident,
--                                <=1 per INCIDENTS row via unique INCIDENT_ID)
--
-- Three new governed metrics (exactly three, mirroring the Kobo pattern):
--   Cloud Infrastructure Cost = SUM(DAI_TEAM_APP_MONTHLY_COSTS.CLOUD_COST)        currency
--   Engineering Hours Logged  = SUM(DAI_TEAM_APP_MONTHLY_COSTS.ENGINEERING_HOURS) number
--   Customers Affected        = SUM(DAI_CUSTOMER_IMPACT.CUSTOMERS_AFFECTED)       integer
...
DROP TABLE IF EXISTS "DAI_CUSTOMER_IMPACT";
DROP TABLE IF EXISTS "DAI_TEAM_APP_MONTHLY_COSTS";

-- -------------------------------------------------------------------------------------
-- 1. DAI_TEAM_APP_MONTHLY_COSTS
...
-- -------------------------------------------------------------------------------------
CREATE TABLE "DAI_TEAM_APP_MONTHLY_COSTS" (
    ...
    CONSTRAINT "PK_DAI_COSTS" PRIMARY KEY ("COST_MONTH_ID"),
    CONSTRAINT "UQ_DAI_COSTS_GRAIN" UNIQUE ("TEAM_ID", "APP_ID", "COST_MONTH"),
    CONSTRAINT "CK_DAI_COSTS_BUDGET" CHECK ("BUDGET_CATEGORY" IN ('Run','Grow','Transform')),
    CONSTRAINT "CK_DAI_COSTS_POS" CHECK ("CLOUD_COST" > 0 AND "ENGINEERING_HOURS" > 0)
);

-- -------------------------------------------------------------------------------------
-- 2. DAI_CUSTOMER_IMPACT
...
-- -------------------------------------------------------------------------------------
CREATE TABLE "DAI_CUSTOMER_IMPACT" (
    ...
    CONSTRAINT "PK_DAI_IMPACT" PRIMARY KEY ("IMPACT_ID"),
    CONSTRAINT "UQ_DAI_IMPACT_INCIDENT" UNIQUE ("INCIDENT_ID"),
    CONSTRAINT "CK_DAI_IMPACT_SLA" CHECK ("SLA_STATUS" IN ('Met','Breached')),
    CONSTRAINT "CK_DAI_IMPACT_POS" CHECK ("CUSTOMERS_AFFECTED" > 0)
);

CREATE INDEX "IX_DAI_COSTS_TEAM"  ON "DAI_TEAM_APP_MONTHLY_COSTS" ("TEAM_ID");
CREATE INDEX "IX_DAI_COSTS_APP"   ON "DAI_TEAM_APP_MONTHLY_COSTS" ("APP_ID");
CREATE INDEX "IX_DAI_COSTS_MONTH" ON "DAI_TEAM_APP_MONTHLY_COSTS" ("COST_MONTH");
CREATE INDEX "IX_DAI_IMPACT_APP"  ON "DAI_CUSTOMER_IMPACT" ("APP_ID");
CREATE INDEX "IX_DAI_IMPACT_DATE" ON "DAI_CUSTOMER_IMPACT" ("INCIDENT_DATE");
