...
- **`dataServeMode`:** 119 `in_memory`, 22 `connect_live`, 15 blank. The blank ones are legacy Hyper/MTDI datasets: Finanzas, <telematics-customer> Hyper, Customer Hyper, Agents Hyper, <retailer> Sample Retail Data, <hpc-vendor> Tenant/Cluster Hyper Dataset, etc.
...
- **External data models:** 12 models referenced others — <card-issuer> Comprehensive Analysis Suite referenced 5+ models; also Unified Supplier Model, <industrial-customer> Integrated Asset Health, Cardmember Rewards Model, etc.
...
MSTR_PASSWORD=... /usr/bin/python3 skill/scripts/strategy_mosaic_inventory.py \
  --model-name "<card-issuer>" --out /tmp/mosaic-card-issuer.json
