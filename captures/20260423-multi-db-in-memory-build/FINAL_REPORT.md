...
| URL | https://studio.strategy.com/MicroStrategyLibrary/app/library#/model/7CA52A46A3C64754894BA4EB45BE428A |
| Tables | 4 (Incidents + tenant_service_hourly from Neon; TENANTS + USAGE_HOURLY from WACSE Snowflake) |
...
| Security filter | **"Tenant = NovaForge AI"** (`A5F38FC12A104642A0A21849A2DE79D4`), assigned to **toconnell** (`DBE00854E14B8D8919D3FBADCA61894B`) |
| Folder | Shared Reports/Collaboration/Arpan Sharma (`DC377018BD4CACD81B7E4CAEB8DB62B4`) |

## Validation — tenant-level accuracy

| Metric | Result |
|---|---|
| Row count at (tenant × cluster × service_ts × incident) grain | **17,280 NovaForge rows — MATCHES validation.csv exactly** |
...
| Phase | Elapsed | Notes |
|---|---|---|
| Env discovery + project resolution | ~1 min | list-datasources, find Shared Studio project, find "Arpan Sharma" destination folder |
...
| Security filter create + assign (Shape B element_list, after Shape A 500'd) | ~30 s | `toconnell` resolved via `abbreviationBegins=t` |
...
7. **Security filter DESC form rejected `8004c767`** — `attribute_form_custom` not accepted. Use Shape B element_list (`hNovaForge AI`). Covered in pre-existing `reference_mosaic_security_filter.md`.
