-- =====================================================================================
-- Post-load consistency suite for the EReaderCo extension tables (run in Neon PG, psql).
-- Every check lists violations; a correct load returns 0 rows / expected counts.
-- =====================================================================================

-- 0. Row inventory
SELECT 'EREADER_SUBSCRIPTION_MONTHS' AS t, count(*) AS rows_, count(DISTINCT "CUSTOMER_ID") AS customers
FROM "EREADER_SUBSCRIPTION_MONTHS"
UNION ALL
SELECT 'EREADER_PROMO_REDEMPTIONS', count(*), count(DISTINCT "CUSTOMER_ID")
FROM "EREADER_PROMO_REDEMPTIONS";
...
FROM "EREADER_SUBSCRIPTION_MONTHS" s
WHERE s."SUBSCRIPTION_STATUS" = 'Churned'
  AND s."DATE" <> (SELECT max(s2."DATE") FROM "EREADER_SUBSCRIPTION_MONTHS" s2
                   WHERE s2."CUSTOMER_ID" = s."CUSTOMER_ID");

-- 3. First-of-month convention
SELECT count(*) AS not_first_of_month
FROM "EREADER_SUBSCRIPTION_MONTHS" WHERE extract(day FROM "DATE") <> 1;

-- 4. MRR matches plan price
SELECT DISTINCT "PLAN_TYPE", "MRR_AMOUNT" FROM "EREADER_SUBSCRIPTION_MONTHS"
WHERE ("PLAN_TYPE", "MRR_AMOUNT") NOT IN (VALUES
    ('EReaderCo Plus Read', 7.99::double precision),
    ('EReaderCo Plus Listen', 7.99::double precision),
    ('EReaderCo Plus Read & Listen', 9.99::double precision));
...
FROM "EREADER_SUBSCRIPTION_MONTHS" GROUP BY 1 ORDER BY 1;

-- 7. Campaign mix
SELECT "CAMPAIGN_NAME", "PROMO_TYPE", count(*) AS redemptions,
       round(sum("DISCOUNT_AMOUNT")::numeric, 2) AS total_discount
FROM "EREADER_PROMO_REDEMPTIONS" GROUP BY 1, 2 ORDER BY 3 DESC;
