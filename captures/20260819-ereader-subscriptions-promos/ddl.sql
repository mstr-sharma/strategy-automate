-- =====================================================================================
-- EReaderCo unified business metrics -- subscription & promotion extension tables (PostgreSQL / Neon)
--
-- Extends the Studio Mosaic model
--   "EReaderCo Content & Customer Engagement Model" (20B2F4E3B58A47C7825C2C7A8D0559E9)
-- which holds CONTENT (content grain) / CUSTOMERS (customer grain) /
-- ENGAGEMENT (session grain) / SALES (transaction grain) in Snowflake <snowflake-schema>,
...
--   EREADER_SUBSCRIPTION_MONTHS   (subscriber x calendar month, periodic snapshot)
--   EREADER_PROMO_REDEMPTIONS     (promo redemption == discounted sales transaction)
--
-- Three new governed metrics (exactly three, per request):
--   Subscription Revenue   = SUM(EREADER_SUBSCRIPTION_MONTHS.MRR_AMOUNT)     currency
--   Active Subscribers     = SUM(EREADER_SUBSCRIPTION_MONTHS.IS_ACTIVE_EOM)  integer
--   Promo Discount Amount  = SUM(EREADER_PROMO_REDEMPTIONS.DISCOUNT_AMOUNT)  currency
...
DROP TABLE IF EXISTS "EREADER_PROMO_REDEMPTIONS";
DROP TABLE IF EXISTS "EREADER_SUBSCRIPTION_MONTHS";

-- -------------------------------------------------------------------------------------
-- 1. EREADER_SUBSCRIPTION_MONTHS
--    Grain: one row per subscriber per calendar month with a PAID EReaderCo Plus
...
-- -------------------------------------------------------------------------------------
CREATE TABLE "EREADER_SUBSCRIPTION_MONTHS" (
    ...
    "PLAN_TYPE"             varchar(30)      NOT NULL,  -- 'EReaderCo Plus Read' | 'EReaderCo Plus Listen' | 'EReaderCo Plus Read & Listen'
    "SIGNUP_SOURCE"         varchar(30)      NOT NULL,  -- discovery channel: 'EReaderCo Storefront', 'Email Campaign', ...
    ...
    CONSTRAINT "PK_EREADER_SUBSCRIPTION_MONTHS" PRIMARY KEY ("SUB_MONTH_ID"),
    CONSTRAINT "UQ_EREADER_SUB_CUST_MONTH" UNIQUE ("CUSTOMER_ID", "DATE"),
    CONSTRAINT "CK_EREADER_SUB_STATUS" CHECK ("SUBSCRIPTION_STATUS" IN ('New','Active','Churned')),
    CONSTRAINT "CK_EREADER_SUB_FLAG" CHECK ("IS_ACTIVE_EOM" IN (0,1)),
    CONSTRAINT "CK_EREADER_SUB_FLAG_STATUS" CHECK (("IS_ACTIVE_EOM" = 0) = ("SUBSCRIPTION_STATUS" = 'Churned'))
);

-- -------------------------------------------------------------------------------------
-- 2. EREADER_PROMO_REDEMPTIONS
...
-- -------------------------------------------------------------------------------------
CREATE TABLE "EREADER_PROMO_REDEMPTIONS" (
    ...
    "PROMO_TYPE"        varchar(30)      NOT NULL,      -- 'Percentage Discount', 'Partner Points Redemption', ...
    -- ---- measures ----
    "DISCOUNT_AMOUNT"   double precision NOT NULL,      -- USD discount vs list-derived gross price
    CONSTRAINT "PK_EREADER_PROMO_REDEMPTIONS" PRIMARY KEY ("REDEMPTION_ID"),
    CONSTRAINT "UQ_EREADER_PROMO_TXN" UNIQUE ("TRANSACTION_ID"),
    CONSTRAINT "CK_EREADER_PROMO_DISC" CHECK ("DISCOUNT_AMOUNT" > 0)
);

CREATE INDEX "IX_EREADER_SUB_CUSTOMER" ON "EREADER_SUBSCRIPTION_MONTHS" ("CUSTOMER_ID");
CREATE INDEX "IX_EREADER_SUB_DATE"     ON "EREADER_SUBSCRIPTION_MONTHS" ("DATE");
CREATE INDEX "IX_EREADER_PROMO_CUST"   ON "EREADER_PROMO_REDEMPTIONS" ("CUSTOMER_ID");
CREATE INDEX "IX_EREADER_PROMO_DATE"   ON "EREADER_PROMO_REDEMPTIONS" ("DATE");
