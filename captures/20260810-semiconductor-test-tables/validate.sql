-- Validation suite for the SemiTestCo semiconductor test-analysis extension tables.
-- Every check must return zero offending rows (or the stated expected value).

\echo '== V1  row counts =='
SELECT (SELECT count(*) FROM "SEMITEST_WAFER_TEST_RUNS")           AS wafer_test_runs,
       (SELECT count(*) FROM "SEMITEST_PARAMETRIC_TEST_RESULTS")   AS parametric_results;
...
FROM "SEMITEST_WAFER_TEST_RUNS";

\echo '== V3  parametric FIRST_FAIL_UNITS must sum to the run UNITS_FAILED_FIRST_PASS (expect 0) =='
SELECT count(*) AS violations FROM (
  SELECT r."TEST_RUN_ID"
  FROM "SEMITEST_WAFER_TEST_RUNS" r
  JOIN "SEMITEST_PARAMETRIC_TEST_RESULTS" p ON p."TEST_RUN_ID" = r."TEST_RUN_ID"
...
FROM "SEMITEST_PARAMETRIC_TEST_RESULTS" p
JOIN "SEMITEST_WAFER_TEST_RUNS" r ON r."TEST_RUN_ID" = p."TEST_RUN_ID"
...
SELECT (SELECT count(*) FROM "SEMITEST_PARAMETRIC_TEST_RESULTS" p
          WHERE NOT EXISTS (SELECT 1 FROM "SEMITEST_WAFER_TEST_RUNS" r WHERE r."TEST_RUN_ID"=p."TEST_RUN_ID")) AS orphan_results,
       (SELECT count(*) FROM "SEMITEST_WAFER_TEST_RUNS" WHERE "UNITS_TESTED" <= 0)                            AS empty_runs;
...
FROM "SEMITEST_PARAMETRIC_TEST_RESULTS" GROUP BY 1 ORDER BY 1;
...
FROM "SEMITEST_WAFER_TEST_RUNS" GROUP BY 1 ORDER BY 1;
...
FROM "SEMITEST_WAFER_TEST_RUNS" GROUP BY 1,2 ORDER BY 1, 4 DESC NULLS LAST;
