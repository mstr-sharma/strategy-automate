# Model review — "NI Production Quality and Maintenance Model" (2026-08-10)

Model `3F80CD71D1C446E594F200B534F7FCD2`, Shared Studio > My Reports > NI.
6 tables / 53 attributes / 56 fact metrics, `dataServeMode: in_memory`.

Reviewed after the two Neon Postgres tables (`NI_WAFER_TEST_RUNS`,
`NI_PARAMETRIC_TEST_RESULTS`) were added to the model.
...
6. **Add a calibration-age band** so the headline cross-store insight can be charted as a
   ...
   ```sql
   ALTER TABLE "NI_WAFER_TEST_RUNS" ADD COLUMN "CALIBRATION_AGE_BAND" varchar(16);
   UPDATE "NI_WAFER_TEST_RUNS" SET "CALIBRATION_AGE_BAND" =
