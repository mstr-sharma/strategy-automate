# NI semiconductor test-analysis extension tables (2026-08-10)

Two deep-technical PostgreSQL tables in Neon that extend the Studio Mosaic model
**"SD Manufacturing NI Production Quality and Maintenance Model"**
(`3F80CD71D1C446E594F200B534F7FCD2`, Shared Studio > My Reports > NI) for a
National Instruments Test & Measurement demo (Optimal+ displacement).
...
data an Optimal+ user actually works in.
...
other, and both join straight back to the Studio dimensions. `NI_PARAMETRIC_TEST_RESULTS`
...
psql "$CONN" -c "\copy \"NI_WAFER_TEST_RUNS\" FROM 'ni_wafer_test_runs.csv' WITH (FORMAT csv, HEADER true, NULL '')"
psql "$CONN" -c "\copy \"NI_PARAMETRIC_TEST_RESULTS\" FROM 'ni_parametric_test_results.csv' WITH (FORMAT csv, HEADER true, NULL '')"
