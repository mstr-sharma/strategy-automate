-- =====================================================================================
-- NI Semiconductor Test Analysis -- deep-technical extension tables (PostgreSQL / Neon)
--
-- Extends the Studio Mosaic model
--   "SD Manufacturing NI Production Quality and Maintenance Model" (3F80CD71D1C446E594F200B534F7FCD2)
...
--     -> NI_WAFER_TEST_RUNS (wafer/sublot x insertion)
--          -> NI_PARAMETRIC_TEST_RESULTS (test run x test number)
--
...
DROP TABLE IF EXISTS "NI_PARAMETRIC_TEST_RESULTS";
DROP TABLE IF EXISTS "NI_WAFER_TEST_RUNS";

-- -------------------------------------------------------------------------------------
-- 1. NI_WAFER_TEST_RUNS
...
-- -------------------------------------------------------------------------------------
CREATE TABLE "NI_WAFER_TEST_RUNS" (
    ...
    CONSTRAINT "PK_NI_WAFER_TEST_RUNS" PRIMARY KEY ("TEST_RUN_ID"),
...
-- -------------------------------------------------------------------------------------
CREATE TABLE "NI_PARAMETRIC_TEST_RESULTS" (
    "PARAM_RESULT_ID"             varchar(32)   NOT NULL,   -- PK
    "TEST_RUN_ID"                 varchar(24)   NOT NULL,   -- FK -> NI_WAFER_TEST_RUNS
    ...
    CONSTRAINT "PK_NI_PARAMETRIC_TEST_RESULTS" PRIMARY KEY ("PARAM_RESULT_ID"),
    CONSTRAINT "FK_PTR_RUN" FOREIGN KEY ("TEST_RUN_ID") REFERENCES "NI_WAFER_TEST_RUNS" ("TEST_RUN_ID"),
...
-- -------------------------------------------------------------------------------------
CREATE INDEX "IX_WTR_DISPOSITION" ON "NI_WAFER_TEST_RUNS" ("DISPOSITION_ID");
CREATE INDEX "IX_WTR_CUST_PROD"   ON "NI_WAFER_TEST_RUNS" ("CUSTOMER_ID", "PRODUCT_ID");
CREATE INDEX "IX_WTR_DATE"        ON "NI_WAFER_TEST_RUNS" ("TEST_DATE");
CREATE INDEX "IX_WTR_TESTER"      ON "NI_WAFER_TEST_RUNS" ("TESTER_ID");

CREATE INDEX "IX_PTR_RUN"         ON "NI_PARAMETRIC_TEST_RESULTS" ("TEST_RUN_ID");
CREATE INDEX "IX_PTR_DISPOSITION" ON "NI_PARAMETRIC_TEST_RESULTS" ("DISPOSITION_ID");
CREATE INDEX "IX_PTR_CUST_PROD"   ON "NI_PARAMETRIC_TEST_RESULTS" ("CUSTOMER_ID", "PRODUCT_ID");
CREATE INDEX "IX_PTR_DATE"        ON "NI_PARAMETRIC_TEST_RESULTS" ("TEST_DATE");
CREATE INDEX "IX_PTR_TEST"        ON "NI_PARAMETRIC_TEST_RESULTS" ("TEST_NAME");
