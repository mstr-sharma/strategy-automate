...
- No native skills protocol — each `SKILL.md` under `skill/`, `strategy-automation/`, `strategy-validation/` is a Markdown instruction file. Load whichever matches the user's ask based on its `description` frontmatter.
- Bash/shell tool calls execute `skill/scripts/build_mosaic.py` and siblings. These are plain `requests`-based Python with no LLM-specific dependencies.
